...
The five marks you can leave on a comment are drawn the same way, for the same
reason the rest of the app has no emoji in it: an emoji is somebody else's
picture, redrawn by every phone and every OS update, and it would be the one
thing on screen not speaking in this app's voice. `heart`, `fire`, `bolt` and
`star` are hand-drawn silhouettes; `nod` reuses the kudos thumb, because a
plain yes already has a glyph here. What the database stores is the name of a
mark — `heart`, `fire` — never a character, so redrawing one changes every
reaction ever left without touching a row.
...
something you said, a mark left on something you said, your partner finishing
...
your partner rather than you — *Aarya replied to your comment*, *Aarya loved
your comment* — and both carry the line they landed on as the body.
...
- **iOS needs a gesture before any audio starts**, and grants it to the element
  ...
  writes the preference; only a deliberate tap on the sound control does. That
  gap is exactly what the chip is for: refused, dead preview URL or genuinely
  muted, the frame shows a crossed speaker rather than nothing, and one tap on
  it is both the gesture iOS was holding out for and the retry.
...
adding a fourth: a thread whose marks arrive a beat after the words they belong
to reads as a bug, and two people's whole challenge is a few hundred rows.
...
- `comment_reactions` — the marks left on a single comment, keyed
  `(comment_id, from_profile, token)`. The primary key *is* the toggle: the
  same mark twice takes it back. `token` is a name (`heart`, `fire`, `bolt`,
  `star`, `nod`), never a character — see **Design**
...
- Captions live under `caps` in the day's existing `day_tasks.tasks` JSONB, keyed by the photo's own slot (`{ 'diet#1750…': { nm:'Blueberries', v:'loved' } }`) — the same trick `notes` uses, so this needed no schema change and rides the save that was already happening. Reads fall back to `meals`, where captions lived while they were diet-only; a day rewrites itself to `caps` the next time it is saved, so the fallback empties itself out. Deleting a photo takes its caption with it rather than leaving an orphan.
- A caption falls back to the task's written note when the frame has none of its own. A workout note *is* the caption for its photo, and asking for the same sentence twice would be daft. Only a single-photo task can lend one, so a day of meals never shares a caption between frames.
- `meal:true` now marks only the tasks that get a verdict — captions are universal, ratings are for food.
...
- Opening on the Feed means opening with no gesture spent, so the first attempt at playback is refused every launch and the app waits for a touch. This used to be said in a toast shown once per device; the sound chip says it on the frame instead, in the place you would tap anyway, so the tip and its `soundHint` flag are gone.
- The chip's tap is not a plain toggle, because the glyph does not always mean the same thing. Muted by choice, a tap flips the preference. Silent because autoplay was refused, the preference is already *on* — flipping it would mute something that was never audible — so a tap there is a retry instead. The gesture listener that arms the refusal case deliberately ignores pointers landing on the chip: `pointerdown` runs before the button's own `click`, so letting it through would start the song and let the click stop it again, in one tap.
- The chip is updated in place rather than re-rendered. Replacing the node restarts its transition from nothing, which on a control that shows and hides twice a scroll is a fade that never fades. Its face is swapped only when the state actually changes, for the same reason one level down: rewriting that markup on every paint would restart the bars from flat each time the feed scrolled.
- Audible is the moving bars rather than a speaker glyph, and silent is the glyph. A still icon can say "sound is on"; only a moving one says it is on *now* — and when there is nothing to hear, an animation that has stopped and one that has broken look identical, so the state carrying a reason gets the drawing instead. The bars stop animating once the chip is off screen; there is nothing to see, so there is nothing to run.
- The viewer's scrim ramps up early rather than only at the very bottom, so the caption stays readable over a bright photo; the middle of the gradient is left alone, since this is a photo viewer first.
...
- Neither state of the chip disappears, which is the one place this departs from the feed apps it borrows from. They can fade theirs to nothing because tapping the video toggles sound; here a tap on the collage opens the viewer, so a chip that vanished would leave that post with no way to reach the sound at all — you would have to scroll to another day and back. What retires is the *volume* of the statement, not the mark: silence settles to a whisper (`.24`), and playing stays at full, because bars moving in time with the song are the point rather than a notice to be dismissed.
- The chip carries no border or backing. The name and the date sit up there as bare marks on the shade gradient, and a boxed control among them reads as chrome — the 24px square is a thumb-sized target around a 13px drawing, not a shape anyone should see. That also means the gap to the date is set to 6px rather than 12: the target is wider than the glyph, so the space that *looks* right is measured to the drawing, not to the button's edge.
