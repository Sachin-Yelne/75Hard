...
- A muted chip recedes to a low opacity instead of disappearing outright, which is the one place this departs from the feed apps it borrows from. They can fade theirs to nothing because tapping the video toggles sound; here a tap on the collage opens the viewer, so a chip that vanished would leave a partner's silent post with no way back to audio at all. Audible does fade to nothing — there is nothing left to explain once you can hear it.
