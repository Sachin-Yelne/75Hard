...
Inside a thread almost nothing is boxed. An outline around every count, every
key and every tag turned a handful of quiet facts into a row of little windows,
and a conversation is mostly quiet facts — so space does the separating, and
the one thing a border is still spent on is saying which reactions are yours.
